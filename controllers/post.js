...
const deleteSingleImage = async (req, res) => {
    ...
    try {
        ...
        let updatedPost = await Post.findOneAndUpdate({ _id: postId }, { images: newImagesList }, { returnDocument: "after" })
        res.status(200).json({ status: 'updated', post: updatedPost })
...
const updateTrending = async (req, res) => {
    ...
    try {
        ...
        let posts = await Post.find({}).sort("-monthlyViews").limit(5);
        res.status(200).json({ posts });
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
...
module.exports = {
    getPosts, getSinglePost, updatePost, savePost,
    deletePost, deleteSingleImage, updateTrending, searchPost, getUserPosts, getFilteredPosts
}